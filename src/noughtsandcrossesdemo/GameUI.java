...
public class GameUI extends javax.swing.JFrame {
    ...
    Game gam;
    public GameUI(boolean isAgainstComputer,boolean isEasy) {
        easyComp=isEasy;
    ...
    private void addComputerMove(boolean isHard){
        ...
        if(isHard==false){
            ...
            int[][]allAvailable = gam.getAllAvailableMoves();
            int y =rand.nextInt(allAvailable.length);
            move[0]=allAvailable[y][0];move[1]=allAvailable[y][1];
            addMove(move);
            return;
        }else{
            addMove(gam.getBestMove());
            return;
        }
    }
    public void updateUI(int[][]state){
        for(int y=0;y<state.length;y++){
            for(int x =0;x<state[0].length;x++){
                if(state[y][x]==gam.NOUGHT_VALUE){
                    board[y][x].setText("X");
                }else if(state[y][x]==gam.CROSS_VALUE){
                    board[y][x].setText("0");
                }
    ...
    private void addMove(int[]yThenX){
        int result =gam.addMove(yThenX);
        updateUI(gam.getBoard());
        if(result==gam.LOSS_FOR_NOUGHT_VALUE){
            JOptionPane.showMessageDialog(this, "Noughts Win!", "Game Over", JOptionPane.OK_OPTION);
            MenuUI menu = new MenuUI();
            menu.setVisible(true);
            this.dispose();
        }else if(result == gam.WIN_FOR_NOUGHT_VALUE){
            JOptionPane.showMessageDialog(this, "Crosses Win!", "Game Over", JOptionPane.OK_OPTION);
            MenuUI menu = new MenuUI();
            menu.setVisible(true);
            this.dispose();
        }else if(result==gam.DRAW_VALUE){
            JOptionPane.showMessageDialog(this, "It's a draw!", "Game Over", JOptionPane.OK_OPTION);
            MenuUI menu = new MenuUI();
            menu.setVisible(true);
            this.dispose();
        }else{
            if(isAgainstComp==true){
                if(playerOneIsComp==true&&gam.getMoveNumber()%2==1||playerOneIsComp==false&&gam.getMoveNumber()%2==0){
    ...
    private void lbl21MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_lbl21MouseClicked
        int []yThenX={2,1};
        if(gam.getBoard()[yThenX[0]][yThenX[1]]==gam.EMPTY_SPACE_VALUE){
            addMove(yThenX);
        }
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new GameUI(isAgainstComp,easyComp).setVisible(true);
            }
        });
    }
